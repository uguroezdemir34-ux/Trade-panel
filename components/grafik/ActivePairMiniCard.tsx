 ...
 * kartlarındaki AYNI veri kaynaklarının (QX halkası + 15M/1H/4H/1D yön
 ...
 * computeDisplayTrends) doğrudan okuyan bağımsız, küçük bir component —
 * ana karar ekranında sıfır regresyon riski.
 *
 * Layout: ScoreRingV2 üstte, 15M/1H/4H/1D okları ALTINDA (dikey grup),
 ...
 * hesaplanmış ScoreResult) okunuyor. computeDisplayTrends (lib/market/)
 * saf, skor motorunun dışında (GuardianPanel'in kullandığı AYNI fonksiyon
 * — market kartları/GuardianPanel ile tutarsızlığı gidermek için buraya
 * da bağlandı, computeMtfTrend'in kendisi/skor gate'i hiç değişmedi).
 */
...
export function ActivePairMiniCard({ pair }: Props): React.ReactElement {
  ...
  // GuardianPanel'deki AYNI computeDisplayTrends() — yeni bir hesaplama
  // yazılmadı. 15M/1H/4H/1D — market kartları ve GuardianPanel ile tutarlı.
  const displayTrends = useMemo(
    () => computeDisplayTrends(candles15m, candles1h, candles4h, candles1d),
