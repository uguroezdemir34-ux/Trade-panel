...
export function GateioCredsCard(): React.ReactElement {
  return (
    <SubscriptionGate feature="multiExchange">
      <GateioCredsCardInner />
    </SubscriptionGate>
  );
}

function GateioCredsCardInner(): React.ReactElement {
  const t = useT();
  const gateioFutures = useCredentialStore((s) => s.gateioFutures);
  const setGateioFutures = useCredentialStore((s) => s.setGateioFutures);
  ...
  async function handleSave() {
    ...
    setSaving(true);
    await setGateioFutures({ key: key.trim(), secret: secret.trim() });
    setSaving(false);
    setSaved(true);
    setKey("");
    setSecret("");
  ...
  async function handleClear() {
    await setGateioFutures(null);
    if (activeExchange === "gateio") setActiveExchange("okx");
  }

  const isActive = activeExchange === "gateio";
    ...
    >
      ...
      <div className="flex items-center justify-between">
        <div>
          <p className="font-mono text-sm font-semibold text-text-t1">
            {t("settings.gateioCreds.title")}
          </p>
          <p className="mt-0.5 font-mono text-xs text-text-t3">
            {t("settings.gateioCreds.subtitle")}
          </p>
          {!EXECUTION_ENABLED && (
        ...
        {gateioFutures && (
          <button
            type="button"
            onClick={() => setActiveExchange(isActive ? "okx" : "gateio")}
            className={[
          ...
          >
            {isActive ? t("settings.gateioCreds.active") : t("settings.gateioCreds.setActive")}
          </button>
        )}
      </div>
      ...
      {gateioFutures ? (
        ...
        <div className="mt-3 space-y-2">
          <input
            type="password"
            placeholder={t("settings.gateioCreds.keyPlaceholder")}
            value={key}
          ...
          <input
            type="password"
            placeholder={t("settings.gateioCreds.secretPlaceholder")}
            value={secret}
          ...
          >
            {!unlocked
              ? "🔒 PIN Kilidini Açın"
              : saved
                ? "✓ " + t("settings.gateioCreds.saveSuccess")
                : t("settings.gateioCreds.save")}
          </button>
        </div>
      )}
