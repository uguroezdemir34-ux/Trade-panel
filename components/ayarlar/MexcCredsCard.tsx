...
export function MexcCredsCard(): React.ReactElement {
  return (
...
function MexcCredsCardInner(): React.ReactElement {
  const t = useT();
  const mexcFutures = useCredentialStore((s) => s.mexcFutures);
  const setMexcFutures = useCredentialStore((s) => s.setMexcFutures);
  ...
  async function handleSave() {
    ...
    setSaving(true);
    await setMexcFutures({ key: key.trim(), secret: secret.trim() });
    setSaving(false);
    setSaved(true);
    setKey("");
    setSecret("");
  ...
  async function handleClear() {
    await setMexcFutures(null);
    if (activeExchange === "mexc") setActiveExchange("okx");
  }

  const isActive = activeExchange === "mexc";
    ...
    >
      ...
      <div className="flex items-center justify-between">
        <div>
          <p className="font-mono text-sm font-semibold text-text-t1">
            {t("settings.mexcCreds.title")}
          </p>
          <p className="mt-0.5 font-mono text-xs text-text-t3">
            {t("settings.mexcCreds.subtitle")}
          </p>
          {!EXECUTION_ENABLED && (
        ...
        {mexcFutures && (
          <button
            type="button"
            onClick={() => setActiveExchange(isActive ? "okx" : "mexc")}
            className={[
          ...
          >
            {isActive ? t("settings.mexcCreds.active") : t("settings.mexcCreds.setActive")}
          </button>
        )}
      </div>
      ...
      {mexcFutures ? (
        ...
        <div className="mt-3 space-y-2">
          <input
            type="password"
            placeholder={t("settings.mexcCreds.keyPlaceholder")}
            value={key}
          ...
          <input
            type="password"
            placeholder={t("settings.mexcCreds.secretPlaceholder")}
            value={secret}
          ...
          >
            {!unlocked
              ? "🔒 PIN Kilidini Açın"
              : saved
                ? "✓ " + t("settings.mexcCreds.saveSuccess")
                : t("settings.mexcCreds.save")}
          </button>
        </div>
      )}
