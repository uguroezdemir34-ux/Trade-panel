...
/**
 * Spike candle detector — son mumun hareketi 20 mumun ortalamasını ≥2.5× aşıyorsa
 * yükseliş momentum zirvesinde giriş riskini işaret eder.
 * closes: 1H kapanışlar, en yenisi sonda. En az 22 mum gerekir.
 */
export function checkSpikeCandle(closes: readonly number[]): string | null {
  if (closes.length < 22) return null;
  const last = closes[closes.length - 1];
  const prev = closes[closes.length - 2];
  if (prev <= 0) return null;
  const lastMovePct = Math.abs(last - prev) / prev;

  let sum = 0;
  for (let i = closes.length - 22; i < closes.length - 2; i++) {
    const c0 = closes[i];
    const c1 = closes[i + 1];
    if (c0 > 0) sum += Math.abs(c1 - c0) / c0;
  }
  const avgMovePct = sum / 20;
  if (avgMovePct <= 0) return null;

  const ratio = lastMovePct / avgMovePct;
  if (ratio >= 2.5) {
    return `📈 Spike candle (${ratio.toFixed(1)}× avg) — retest bekle`;
  }
  return null;
}

/**
 * EMA20 mesafe filtresi — fiyat EMA20'den >%3 uzaktaysa aşırı uzanmış sayılır.
 * px: anlık fiyat, ema20: 1H EMA21, direction: işlem yönü.
 */
export function checkEmaDistance(
  px: number,
  ema20: number | null,
  direction: Direction,
): string | null {
  if (ema20 === null || ema20 <= 0) return null;
  const distPct = ((px - ema20) / ema20) * 100;
  if (direction === "LONG" && distPct > 3.0) {
    return `📏 Fiyat EMA20 üzerinde +${distPct.toFixed(1)}% — aşırı uzanmış, pullback bekle`;
  }
  if (direction === "SHORT" && distPct < -3.0) {
    return `📏 Fiyat EMA20 altında ${distPct.toFixed(1)}% — aşırı uzanmış, pullback bekle`;
  }
  return null;
}
