...
export function simulateExit(
  ...
  for (let i = 0; i < limit; i++) {
    ...
    if (slHit) {
      return { exitTs: c.ts, exitPrice: stopPrice, exitReason: "sl", barsHeld: i + 1 };
    }

    // TP2 check (more aggressive target)
    const tp2Hit = isLong ? c.high >= tp2Price : c.low <= tp2Price;
    if (tp2Hit) {
      return { exitTs: c.ts, exitPrice: tp2Price, exitReason: "tp2", barsHeld: i + 1 };
    }

    // TP1 check
    const tp1Hit = isLong ? c.high >= tp1Price : c.low <= tp1Price;
    if (tp1Hit) {
      return { exitTs: c.ts, exitPrice: tp1Price, exitReason: "tp1", barsHeld: i + 1 };
    }
  }
